...
- perform ***null* checks**, e.g. fields can require that they will never get the value *null* (see [*null checks*](#null-checks)),
- **call other methods on changes**, e.g. in order to trigger additional effects (see [Change Methods](#change-methods)),
- synchronize with an "opposite" field in case of **bidirectional connections** (see [Bidirectional Connections](#bidirectional-connections)).
...
In contrast to the common pattern, the generated setter method will return a *boolean* value. A result of *true* means that the actual value of the field has been changed. On contrary, *false* means that the value has not been changed. This can happen if the new value (or rather reference in case of objects) would have been the same as the old one. However, there are also other possibilities that the value is not changed in context of [change methods](#change-methods).
...
| ***boolean putAllToX(Map&lt;? extends K,? extends V&gt; m)*** | *multiple* | *Map* | all entries of map *m* will be put to map<nobr>&nbsp;<nobr>*X* |
...
| ***boolean clearX()*** | *multiple* | *Collection*<nobr>&nbsp;<nobr>/<br>*Map* | the whole collection / map *X* will be emptied |

All methods which return a *boolean* value will report if there has been any change in the *Collection* or *Map*. Even if an element shall be added, *false* can be returned. This can happen, for example, if the new element has already been contained in a *Set*. However, there are also other possibilities to watch and control changes in context of [change methods](#change-methods).
...
> Please note, that no exception will be thrown if an element is not added due to restrictions. On the contrary, for collection classes themselves and their *add* methods such a behavior is expected, for example (see [java.util.Collection](https://docs.oracle.com/javase/8/docs/api/java/util/Collection.html)). 
...
| Annotation Parameter     | Compatible<br>Types  | Parameters of Called Method                   |
|---------------|:------------:|-------------------------------------------------:|
| *beforeChange*&nbsp;/<br>*afterChange* | all | -
| " | " | *T newValue*
| " | " | *T oldValue, T newValue*
| " | " | *String fieldName, T oldValue, T newValue*
| *beforeAdd*&nbsp;/<br>*afterAdd*&nbsp;/<br>*beforeRemove*&nbsp;/<br>*afterRemove* | *java.util.Collection*&nbsp;/<br>*java.util.Map* | -
...
| *beforeAdd*&nbsp;/<br>*beforeRemove* | *java.util.Collection*<br>**not:** *java.util.List* | *List&lt;T&gt; oldElements, List&lt;T&gt; elements*
...
| *afterAdd*&nbsp;/<br>*afterRemove* | *java.util.Collection*<br>**not:** *java.util.List* | *List&lt;T&gt; oldElements, List&lt;T&gt; newElements, List&lt;T&gt; elements*
...
| *beforeElementAdd*&nbsp;/<br>*afterElementAdd*&nbsp;/<br>*beforeElementRemove*&nbsp;/<br>*afterElementRemove* | *java.util.Collection* | *T element*
...
| *beforeElementAdd*&nbsp;/<br>*afterElementAdd* | *java.util.Map* | *Map.Entry&lt;K,V&gt; replacedElement, Map.Entry&lt;K,V&gt; element*
| " | " | *String fieldName, Map.Entry&lt;K,V&gt; replacedElement, Map.Entry&lt;K,V&gt; element*
| *beforeElementAdd*&nbsp;/<br>*beforeElementRemove* | *java.util.Collection*<br>**not:** *java.util.List* | *List&lt;T&gt; oldElements, T element*
...
| *afterElementAdd*&nbsp;/<br>*afterElementRemove* | *java.util.Collection*<br>**not:** *java.util.List* | *List&lt;T&gt; oldElements, List&lt;T&gt; newElements, T element*
